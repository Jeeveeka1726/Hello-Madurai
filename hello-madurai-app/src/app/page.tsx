    ...
    const fetchData = async () => {
      try {
        ...
        const [featuresResponse, newsResponse] = await Promise.all([
          fetch('/api/home-features', { cache: 'no-store' }), // Always fresh features
          fetch('/api/news/latest', { cache: 'no-store' }) // Always fresh news - fixes Firefox caching
        ])
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center bg-white rounded-3xl shadow-xl p-10 md:p-16">
              ...
              <div className="flex justify-center items-center gap-4 md:gap-6">
                ...
                {/* Telegram */}
                <a
                  href="https://t.me/hellomadurai"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group flex items-center justify-center w-14 h-14 md:w-16 md:h-16 bg-[#0088cc] hover:bg-[#0077b3] rounded-xl transition-all duration-300 hover:scale-110 hover:shadow-lg"
                  aria-label="Telegram"
                >
                  <svg className="w-7 h-7 md:w-8 md:h-8 text-white" fill="currentColor" viewBox="0 0 24 24">
