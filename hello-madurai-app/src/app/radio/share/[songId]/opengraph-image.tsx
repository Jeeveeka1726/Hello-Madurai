...
export default async function Image({ params }: { params: Promise<{ songId: string }> }) {
  try {
    ...
    // If artist has an image URL, redirect to it
    if (song.singer.imageUrl) {
      // For Cloudinary images, we can't redirect in ImageResponse
      // So we'll create a composite image with the artist image
      return new ImageResponse(
        (
          <div
            style={{
    ...
    return new ImageResponse(
      (
        ...
        >
          <div style={{ fontSize: 64, fontWeight: 'bold', textAlign: 'center', marginBottom: 20 }}>
            {song.title}
          </div>
          <div style={{ fontSize: 40, textAlign: 'center', marginBottom: 30 }}>
            {song.singer.name}
          </div>
