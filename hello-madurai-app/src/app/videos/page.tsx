...
function VideosPageContent() {
  ...
  // Handle WhatsApp share with thumbnail
  const handleWhatsAppShare = async (video: Video) => {
    ...
    if (navigator.share) {
      try {
        const shareData: any = {
          title: videoTitle,
          text: `${videoTitle} - Hello Madurai`,
          url: shareUrl,
        }

        // Try to include thumbnail image
        const thumbnailUrl = video.thumbnailUrl || (video.videoType === 'youtube' && video.videoUrl ?
          getYouTubeThumbnail(video.videoUrl.split('v=')[1]?.split('&')[0] || video.videoUrl.split('/').pop() || '') :
          null)

        if (thumbnailUrl) {
          try {
            const absoluteUrl = thumbnailUrl.startsWith('http') ? thumbnailUrl : `${window.location.origin}${thumbnailUrl}`
            const response = await fetch(absoluteUrl)
            const blob = await response.blob()
            const file = new File([blob], 'video-thumbnail.jpg', { type: blob.type })

            if (navigator.canShare && navigator.canShare({ files: [file] })) {
              shareData.files = [file]
            }
          } catch (imgError) {
            console.log('Could not fetch thumbnail for sharing:', imgError)
          }
        }
  ...
  // Handle Facebook share with thumbnail
                ...
                <>
                  <Card key={video.id} className="overflow-hidden hover:shadow-xl transition-all bg-white border-gray-200">
                    ...
                    <CardContent className="p-3 sm:p-4">
                      ...
                      {/* Share Buttons - Fully Responsive with Better Touch Targets */}
                      <div className="flex gap-2 sm:gap-2.5">
                        {/* WhatsApp Share */}
                        <button
                          onClick={() => handleWhatsAppShare(video)}
                          className="flex-1 bg-green-500 hover:bg-green-600 active:bg-green-700 active:scale-95 text-white font-medium py-2.5 sm:py-3 px-2 sm:px-4 rounded-lg transition-all duration-200 flex items-center justify-center gap-1.5 sm:gap-2 shadow-sm hover:shadow-md min-h-[44px]"
                          title="Share on WhatsApp"
                          aria-label="Share on WhatsApp"
                        >
                          <svg className="h-5 w-5 sm:h-5 sm:w-5 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                          ...
                          <span className="hidden sm:inline text-sm font-semibold" suppressHydrationWarning>WhatsApp</span>
                        </button>

                        {/* Facebook Share */}
                        <button
                          onClick={() => handleFacebookShare(video)}
                          className="flex-1 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 active:scale-95 text-white font-medium py-2.5 sm:py-3 px-2 sm:px-4 rounded-lg transition-all duration-200 flex items-center justify-center gap-1.5 sm:gap-2 shadow-sm hover:shadow-md min-h-[44px]"
                        ...
                        >
                          ...
                          <span className="hidden sm:inline text-sm font-semibold" suppressHydrationWarning>Facebook</span>
                        </button>
                      </div>
                    </CardContent>
                  </Card>
